...
class HybridDiscretization:
    def __init__(self, path):
        ...
        self.colmap = {1: 'r', 2: 'g', 3: 'b'}
        self.bit = {1: '0', 2: '1', 3: '2'}
        self.root_path = os.path.dirname(path)
        self.filename_none_extension = Path(path).stem

    def execute(self, with_header=True):
        discretization_graph_path = self.directory_making("hybrid/graphs/%s/" % self.filename_none_extension)
        discretization_gene_path = self.directory_making("hybrid/values/")
        result = pd.DataFrame({
        ...
        outpath = '%s/%s.txt' % (discretization_gene_path, self.filename_none_extension)
        if with_header:
    ...
    def K_mean(self, index, x, k, save_path):
        ...
        maxValueOfThisGene = HybridDiscretization.gene_max(x)
        minValueOfThisGene = HybridDiscretization.gene_min(x)
        wcss = []
        list_validity = []
        df = pd.DataFrame({
    ...
    def OptimalK(self, x):
        ...
        return 3 if list_validity[0] > list_validity[1] else 2
